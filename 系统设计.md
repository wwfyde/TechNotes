...
> 系统科学, 系统分析 , 系统架构, 系统工程, 系统理论, 架构设计,软件工程, 架构模式
>
...
数据建模通常涉及概念建模、逻辑建模和物理建模，这是数据库设计和信息系统开发中的核心环节。在大学中，与这些内容相关的知识通常会在以下课程中教授：

### 1. 数据库原理（Database Principles）

- **相关性**：这是最直接与数据建模相关的课程，涵盖了数据库设计的基础知识。

- 内容

  ：

  - **概念建模**：学习使用ER图（实体-关系图）等工具，捕捉业务需求中的实体、属性和关系。
  - **逻辑建模**：将概念模型转化为关系模型，设计数据库的逻辑结构。
  - **物理建模**：将逻辑模型映射到具体的数据库管理系统（DBMS），设计表、索引和存储结构。

- **意义**：学生通过该课程掌握从需求分析到数据库实现的完整数据建模过程。

### 2. 信息系统分析与设计（Information Systems Analysis and Design）

- **相关性**：该课程侧重于信息系统的整体设计，数据建模是其中重要组成部分。
...
- **意义**：帮助学生理解数据建模在信息系统开发中的应用，培养从业务到设计的综合能力。

### 3. 软件工程（Software Engineering）

- **相关性**：虽然主要聚焦软件开发过程，但在需求分析和系统设计阶段会涉及数据建模。
...
- **意义**：强调数据建模在软件项目中的实际应用。

### 4. 数据仓库与数据挖掘（Data Warehousing and Data Mining）

- **相关性**：涉及更高级的数据建模技术，特别是逻辑建模的特定形式。

- 内容

  ：

  - **维度建模（Dimensional Modeling）**：适用于数据仓库设计，关注数据的组织以支持高效查询和分析。

- **意义**：学生可以掌握数据建模在大数据和数据分析领域的应用。

### 5. 面向对象分析与设计（Object-Oriented Analysis and Design）

- **相关性**：使用UML（统一建模语言）进行建模，与概念建模和逻辑建模相关。

- 内容

  ：

  - 使用类图（Class Diagram）将业务需求转化为面向对象的模型，并进一步设计数据库结构。

- **意义**：帮助学生理解面向对象建模与数据建模的联系。

### 总结

- **最核心的课程**：**数据库原理**，全面覆盖概念建模、逻辑建模和物理建模，是学习数据建模的基础。
- **辅助课程**：**信息系统分析与设计**和**软件工程**，将数据建模融入系统开发的更广泛背景。
- **进阶课程**：**数据仓库与数据挖掘**和**面向对象分析与设计**，提供特定领域或高级建模技术。

这些课程通过理论教学和实践项目，帮助学生掌握数据建模的技能，为从事数据库设计、系统开发或数据分析等职业打下坚实基础。
...
| ----------- | -------------- | -------------------------------- |
| emergence   | 涌现           |                                  |
| perspective | 视角/角度/意义 | from a certain angle/perspective |
| sense       | 意义           | in a sense<br />从某种意义上讲   |
| dimension   | 维度/视角      |                                  |
| viewport    | 视窗/观点      |                                  |
| aspect      | 切面/方面      |                                  |
| inspire     | 启发           |                                  |
...
六边形架构, DDD, Clean Architecture

领域层(抽象): domain(DDD)-entity, Domain layer, containing core business logic

应用层(用例): application/service,     # Application layer, combining domain objects and infrastructure implementations

适配器/infra/基础设施: repo,  Infrastructure implementation layer

接口(interface): http, cli, event, 
...
> DAO(data access object)、Service、Controller

**handlers** -> routers  -> **services** -> schemas -> **models**
...
![https://blog.cleancoder.com/uncle-bob/images/2012-08-13-the-clean-architecture/CleanArchitecture.jpg](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202504231435783.jpg)


| **层级 (Layer)**                           | **主要职责 (Responsibility)**                                | **典型术语 (English ⇔ 中文)**                                |
| ------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **Entities（实体）**<br />                 | 企业级业务对象，核心业务规则                                 | Entity ⇔ 实体                                                |
| **Use Cases / Interactors（用例/交互器）** | 封装用例逻辑，协调实体，构成“业务逻辑”                       | Use Case ⇔ 用例<br />Interactor ⇔ 交互器                     |
| **Interface Adapters（接口适配器）**       | 将外部输入（HTTP、UI、CLI）转换为用例请求，和将用例输出转换为外部格式 | Controller ⇔ 控制器<br />Presenter ⇔ 展示器<br />Gateway / Repository ⇔ 网关/仓储 |
| **Frameworks & Drivers（外部框架）**       | UI、数据库、Web 框架、外部 API                               | UI View ⇔ 界面视图DB ⇒ 数据库驱动                            |

- **业务逻辑 (Business Logic)** 就是 **Use Cases / Interactors（用例/交互器）层**，也常被实现为 **Services（服务）**。
- **Controllers（控制器）** 属于 **Interface Adapters（接口适配器）**，它依赖于用例/服务层，将 HTTP 请求、消息、事件等“翻译”成用例调用。
...
- “万变不离其宗”——真正贵的是业务知识与边界设计

- 通过 **分层架构**(Layers Architecture) 实现 **关注点分离**(separarion of concerns)

- 分层的是一个个抽象

- 外圈是机制, 内圈是策略——"The outer circles are mechanisms. The inner circles are policies."

  - <u>越往外圈越接近细节, 也就越低层</u>

- 战略设计三原则: 上下文、提炼和大型结构

- 《代码整洁之道》教你写出易读、可扩展、可维护、可重用的代码，《代码整洁之道：程序员的职业素养》教你怎样变成一个有修养的程序员，而《架构整洁之道》基本上是在描述软件设计的一些理论知识。《架构整洁之道》大体分成三个部分：**编程范式**（结构化编程、面向对象编程和函数式编程），**设计原则**（主要是SOLID），以及**软件架构**（其中讲了很多高屋建翎的内容）。总体来说，这本书中的内容可以让你从微观（代码层面）和宏观（架构层面）两个层面对整个软件设计有一个全面的了解。

- 无论是微观世界的代码，还是宏观层面的架构，无论是三种编程范式还是微服务架构，它们都在解决一个问题——分离**控制**和**逻辑**。所谓控制就是对程序流转的与业务逻辑无关的代码或系统的控制（如多线程、异步、服务发现、部署、弹性伸缩等），所谓逻辑则是实实在在的业务逻辑，是解决用户问题的逻辑。控制和逻辑构成了整体的软件复杂度，有效地分离控制和逻辑会让你的系统得到最大的简化。

- 软件架构关注的三大重点: 功能性, 组件独立性, 数据管理

- 人们可以用顺序结构,分支结构和循环结构构造出任何程序

- 科学与数学,科学理论和科学定律可以被证伪(prove false), 但没法通过数学证明(prove true)
...
- 数据库,Web, 应用程序框架均是实现细节, Web可以将其看作一种I/O设备

- 按层封装(水平), 从技术角度进行分类web层, 业务逻辑层, 持久化层

- 按功能封装(垂直), 从业务角度进行切分, 文件夹代表业务, 文件代表技术角度
...
这种范式的核心本质是 源代码的依赖法关系可以通过引入接口的方式来进行反转
...
需求建模, 领域建模, 概念建模, 逻辑建模, 物理建模, 维度建模, 数据架构, 数据系统设计 data-oriented-design 

**业务建模(business modeling)**: 业务模型, 定义业务蓝图, **业务的边界和规则**。业务流程建模与标注 (BPMN)

需求分析: 在业务模型的基础上，明确了**系统要实现的功能和目标**。

系统分析: 将这些需求转化为 **系统的逻辑结构和对象模型**。实体关系图, 数据流图, 状态机图 . **系统分析模型 (System Analysis Model):** 包括一系列UML图（如类图、顺序图、状态图等）和ER图。

**系统设计**: 则根据逻辑模型，制定出 **具体的物理实现方案和技术架构**。

**信息系统设计**(information system design), 数据建模, 数据抽象, 蕴含, 系统分析设计, 数据建设, 数据架构 抽象数据模型 信息结构

面相数据设计, 数据模型 PO Model  Persistent Object
...
物理建模: 研究数据库在物理存储上的实现，如索引设计、表分区等。

ORM DAL(Data Access Layer)

[什么是数据建模](https://aws.amazon.com/cn/what-is/data-modeling/)

[wiki:数据建模](https://zh.wikipedia.org/wiki/数据建模)

[Data-oriented, clean&hexagonal architecture software in Rust - through an example project](https://dpc.pw/posts/data-oriented-cleanandhexagonal-architecture-software-in-rust-through-an-example/)

[Mongodb 数据建模与序列化](https://www.mongodb.com/docs/drivers/rust/current/fundamentals/serialization/)

[wiki实体关系模型: 讲到 概念数据模型, 逻辑数据模型, 物理数据模型](https://en.wikipedia.org/wiki/Entity–relationship_model#Cardinalities)
...
在[软件工程](https://zh.wikipedia.org/wiki/软件工程)中，**数据建模**（data modeling）是在设计[数据库](https://zh.wikipedia.org/wiki/数据库)时，将现实世界各类[数据](https://zh.wikipedia.org/wiki/数据)及其关系进行分析、抽象，从中找出内在联系，并形式化描述为[数据模型](https://zh.wikipedia.org/wiki/数据模型)，建立[信息系统](https://zh.wikipedia.org/wiki/信息系统)的数据库结构的过程。



| **层次**   | **名称**                       | **描述**                                                     |
| ---------- | ------------------------------ | ------------------------------------------------------------ |
| **概念层** | **概念数据模型（如ER图）**     | 抽象现实世界的实体和关系，不关心技术细节。主要与业务人员沟通。 |
| **逻辑层** | **逻辑数据模型（如关系模型）** | 细化成表、字段、主外键，独立于具体数据库。用于系统分析设计。 |
| **物理层** | **物理数据模型**               | 落实到具体数据库实现（如 MySQL、Oracle），考虑索引、性能等。 |
...
- **《数据库系统概念》**（Database System Concepts）—— 经典教材，覆盖概念、逻辑和物理建模。

  **《数据仓库工具箱》**（The Data Warehouse Toolkit）—— Kimball 的数据仓库建模经典书籍。

  **《UML与模式应用》**（Applying UML and Patterns）—— 对 UML 及概念建模有详细解释。
...
**逻辑建模**（细化实体为表结构并定义主键、外键等约束）和

**物理建模**（结合具体数据库技术实现存储结构和性能优化）

**维度建模** 现代企业计算使用数据仓库技术存储大量数据进行数据分析。您可以通过维度数据建模项目从数据仓库中进行高速数据存储和检索。维度模型使用重复或冗余数据，它会优先考虑性能，而不是使用更少的数据存储空间。

例如，在维度数据模型中，汽车经销商具有“汽车”、“展厅”和“时间”等维度。“汽车”维度具有名称和品牌等属性，而“展厅”维度则具有州、城市、街道名称和展厅名称等层次结构

### 数据建模技术

| **概念**         | **典型别名**                           | **主要用途**                  | **是否带业务规则**       | eee  | **推荐出现位置**        |
| ---------------- | -------------------------------------- | ----------------------------- | ------------------------ | ---- | ----------------------- |
| **PO**           | Data Object（DO）、Entity（在 ORM 里） | 读写数据库、序列化/反序列化行 | **❌**——只反映列          |      | infra / persistence     |
| **Domain Model** | Aggregate / Entity                     | 体现业务不变量、行为方法      | **✅**——充满规则          |      | domain                  |
| **DTO**          | VO (View Object)、Response             | 对外 API / 前端展示           | 部分——可能裁剪或组合字段 |      | application / interface |
...
- [编程范式](https://mp.weixin.qq.com/s/7vnlc4-PcraPLL2bMbIbVw)
...
- 分工, 同步, 互斥
- 并发控制
- 一个日益流行的确保安全并发的方式是**消息传递**（*message passing*），这里线程或 actor 通过发送包含数据的消息来相互沟通。这个思想来源于 [Go 编程语言文档](https://golang.org/doc/effective_go.html#concurrency) 中的口号：“不要通过共享内存来通讯；而要通过通讯来共享内存。”（“Do not communicate by sharing memory; instead, share memory by communicating."） ———[使用消息传递在线程间传送数据](https://kaisery.github.io/trpl-zh-cn/ch16-02-message-passing.html#使用消息传递在线程间传送数据)
- 并发模型
  - 语言级别: 线程, 异步/协程, 进程, 程序/实例
  - 架构级别: 消息驱动, 事件驱动, 分层并发模型
- 分布式与并发
  - 并发是一心多用
  - 分布式是多角色协同, 团队协作, 多级协同
