...
- graphviz
- edraw
- [excaldraw](https://excalidraw.com)
...
3. **事件驱动（Event-driven）**：基于事件循环的非阻塞I/O操作，适用于高IO密集型应用。当I/O或其他异步操作完成时，相应的回调函数被触发执行。
...
1. **微服务架构（Microservices Architecture）**：将应用拆分为一组小的、自治的服务，每个服务在独立的进程中运行，服务之间通过轻量级通信机制（如HTTP REST、消息队列）交互。
