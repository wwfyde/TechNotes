...
- 盒模型
- 物理像素
- 逻辑像素(CSS像素): 软件开发相关的单位
- 设备像素比(DPR:

## 常见问题

> [解决常见的 CSS 问题](https://developer.mozilla.org/zh-CN/docs/Learn_web_development/Howto/Solve_CSS_problems)

- 关于覆盖, 后面的覆盖新的, 而且是逐行覆盖, 而不是快级别覆盖
- 高度约束
  - `height:auto`: 高度由内容撑开, body 默认
  - `height:200px`: 固定高度
  - `height: 100%`: 相对父元素高度
  - `height:100vh`: 相对视口高度
  - `height:1rem`: 相对root字体尺寸
  - `heigh: vmin/vmax`: 相对视口最大边最小边
  - 基于上下文: `align-items: stretch`,  `grid-template-rows`
- 盒子模型是基于display来确定的
- 文档流: absolute, fixed, sticky
- 绝对定位是基于祖先元素来定位的, 即最近位置的一个元素
- 滚动时到顶部/底部时会**过度滚动**(弹性滚动/抖动/过度抖动)
  
  - mac 系统或者 ios 浏览器均有过度滚动(弹性滚动)的情况，后续所有的系统均支持了弹性滚动, 本文探讨如何禁用此特性
  - 非Safari浏览器, 通过使用 `overscroll-behavior: contain` 禁用弹性滚动. 除 safari 外，所有的浏览器均支持(不支持的浏览器本身也不支持弹性滚动)
...
- 自动命名空间映射(Automatic Namespace Mapping)
  - --color-border 
...
- 使用 inset-ring(box-shadow) 进行描边 内描边效果
- outline 进行focus
- border 正常边框
- box-shadow 视觉特效
...
描边样式
inset-ring
inset-ring-gray-950/8 (浅色模式下 8% 不透明度的黑色环)

dark:inset-ring-white/15 (深色模式下 15% 不透明度的白色环)
<button class="inset-ring inset-ring-gray-950/8 dark:inset-ring-white/15">one up(inset-ring)
</button>
<button class="outline-1 outline-gray-950/8 -outline-offset-1 dark:outline-white/15">
  one up(outline)
</button>
```
