...
> 中文世界里常常称视觉设计(visual design)为平面设计
>
> 相关概念: 图形设计, 视觉设计,界面设计, 图表设计, 设计语言, UI设计, 数字设计
>
> 计算机图形学
>
...
alpha通道中(0-255), 0表示黑色(暗, 透明), 255表示白色(亮, 不透明), 也可以说成是亮度(灰度), 

**PNG alpha 通道的 0 表示完全透明，255 表示完全不透明。**, 透明度, 像素可见度

分割任务重(0-255), 0表示背景(黑色, 丢弃), 255表示前景(白色, 选取对象)
...
![Unknown](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202505221600269.jpg)
...
![图像](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202409301931784.jpeg)
...
```css
:root{
  font: system-ui,-apple-system, BlinkMacSystemFont, "Segoe UI", "Noto Sans",Roboto,"Helvetica Neue", Arial, sans-serif, "Apple Color Emoji","Segoe UI Emoji";
  /*MacOS SFMono Menlo; Windows: Consolas; Linux: "Liberation Mono", monospace; */
  --monospace: ui-monospace,SFMono, Consolas, "Liberation Mono","Courier New", monospace;
}
```
...
![A selection of shapes available in draw.io commonly used in flow charts](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202408031347158.png)
...
```markdown
[visualising-software-architecture](https://medium.com/@alastairallen/visualising-software-architecture-why-its-important-and-how-i-do-it-5c2f4b65b31a)

Drawing Tools:

- Draw.io: A web and desktop-based tool known for its simplicity and versatility in diagramming.
- Lucidchart: A more advanced set of features when compared to Draw.io, including data, automation, and team collaboration.
- OmniGraffle: A powerful diagramming application for Mac, known for its precision and wide range of styles.

2. Drawing Frameworks:

- UML (Unified Modeling Language): A standardised modelling language that provides a general way to visualise the design of a system.
- C4 (Context, Containers, Components, and Code): A framework that breaks down the architecture into different levels of granularity.
- Archimate: An open standard for enterprise architecture modelling, that supports the description, analysis, and visualisation of architecture.
...
[drawio 实体关系图](https://www.lucidchart.com/pages/er-diagrams)
...
[实体关系模型](https://en.wikipedia.org/wiki/Entity–relationship_model#Cardinalities)

[教程](http://www.woshipm.com/pd/583978.html)



推荐 Crow’s Foot Notation 注解

[![image](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202505221105916.png)](https://user-images.githubusercontent.com/2719310/29148000-7c8d3dd8-7d1f-11e7-9e91-2caf5074f6af.png)

|      | one                                |      |
| ---- | ---------------------------------- | ---- |
| 0    | zero<br />(不会存在于图中, 没意义) |      |
| 1    | one                                |      |
| 3    | many                               |      |
|      | zero or many                       |      |
|      | one or many                        |      |
...
> flowchart
>
> - [基础流程图](https://www.lucidchart.com/pages/what-is-a-flowchart-tutorial)



> - [Flowchart Guides](https://www.zenflowchart.com/guides)
...
![Switch Case Flowchart](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202404191520992.png)

### for-loop

https://www.zenflowchart.com/blog/for-loop-flowchart

![for loop flowchart](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202404261616863.png)

### while-loop

![while loop flowchart](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202404261617114.png)
...
![](https://wwfyde.oss-cn-hangzhou.aliyuncs.com/images/202505221551920.png)
...
1. 色彩调整, 采用统一的色彩主题, 选择几种主色 辅色等等, 按照规律, 规则排列
2. 对齐和分布
3. 布局调整
4. 字体和文本格式
5. 图形大小和形状
...
```text
drawio画好基本的流程图底稿后, 如何美化, 如色彩, 对齐, 布局等等, 有专门的优化工具等等, 来一键美化相应的Diagram吗

色彩调整：
选择和应用统一的色彩主题，以确保图表看起来协调。
使用不同的颜色来区分不同类型的节点或流程，但要避免使用过多鲜艳或刺眼的颜色。
对齐和分布：
使用对齐工具确保图形和文本框的对齐。
使用分布工具来确保图形之间的间距一致。
布局调整：
根据流程图的内容和目的选择合适的布局。例如，对于顺序流程，可以使用垂直或水平线性布局。
调整图形之间的连接线，使其尽可能直接和简洁。
字体和文本格式：
选择清晰易读的字体。
确保文本大小一致，并适当调整以提高可读性。
图形大小和形状：
调整图形的大小，使其既能容纳文本，又不显得过于拥挤或空旷。
使用不同形状来表示不同类型的节点，例如圆形用于开始/结束点，矩形用于标准步骤。
添加图例和说明：
如果使用了特殊的符号或颜色，可以添加图例来解释它们的含义。
在需要的地方添加注释或说明。
```
...
---
graph LR
A[Why-核心观点] --> B[How-论证逻辑]
B --> C[What-具体数据]

classDef front fill:#696,stroke:#333, stroke-width:0;
%%class A front

style B fill:#F05,color:white
style C fill:#F05,color:white

```

```mermaid
%%{init: {"themeVariables": { "stroke": "none"}}}%%
flowchart TD
A[(开始)] --> B


style A fill:#f05,stroke:none, color:white, width:200px
```

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'nodeBorder': 0 }}}%%
graph TD
    A[Node A] --> B[Node B]
    B --> C[Node C]
```
