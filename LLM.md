...
- 辛顿: 智能的本质在于推理(the essence of intelligence is reasoning)——逻辑启发范式(the logic-inspired approach)
  
- 大模型本质上是一种概念分分布, 学习语言的概率分布
  
  - 大模型通过条件概率来预测下一个词出现的概率
...
- 概率分布, 参数不是显式存在的, 而是基于统计规律的数值表示
- 学习目标: 下一个token的概率分布, 学习的是规律
- 大模型的基本原理
  - 讓機器學會產生複雜而有結構的物件
    - 让机器学习生成复杂而且结构化的内容
  - 复杂表示无限的可能
  - 有结构表示有限的选择, 有限的选择有表示这本质上是一个分类问题
    - 而分类问题, 意味着寻找规律
- 大模型通过反复调整参数(连接权重)来使得预测接近真实文本

- llm的参数是透过, 资料自动学习 而非人工设定
  - 参数是连接权重
  - 参数是一个复杂的概率还是得系数的集合
- token是模型处理的最小单位, 也就是词元,(象征,标记)



- **参数与 token 的关系**：
  - 参数通过一个「嵌入层（embedding layer）」把 token ID 映射到向量空间；
  - 然后通过各层计算决定下一个 token 的概率；
  - 训练的过程就是反复调整参数，使输出概率更符合训练语料。
- 学习的主要数据来源:
  - 网络资料
  - 标注数据
  - 使用者反馈
